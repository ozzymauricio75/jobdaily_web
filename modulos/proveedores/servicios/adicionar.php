...
if (!empty($url_generar)) {
    ...
    $codigo = SQL::obtenerValor("servicios","MAX(codigo)","codigo>0");
    if ($codigo){
        $codigo++;
    } else {
        $codigo = 1;
...
/*** Validar los datos provenientes del formulario ***/
    ...
    if ($url_item == "descripcion") {
        $existe = SQL::existeItem("servicios", "descripcion", $url_valor, "codigo > 0");
    ...
    /*** Validar codigo ***/
    if ($url_item == "codigo") {
        $existe = SQL::existeItem("servicios", "codigo", $url_valor, "codigo > 0");
...
/*** Adicionar los datos provenientes del formulario ***/
    ...
    if(empty($forma_codigo)){
    ...
    }elseif($existe = SQL::existeItem("servicios", "codigo", $forma_codigo)){
	    $error   = true;
        $mensaje = $textos["ERROR_EXISTE_CODIGO"]; 
        
    }elseif($existe = SQL::existeItem("servicios", "descripcion", $forma_descripcion)){
	    $error   = true;
        ...
        $insertar = SQL::insertar("servicios", $datos);
