...
if (!empty($url_generar)) {
	...
	$formularios["PESTANA_GENERAL"] = array(
	    ...
	    array(
            HTML::campoTextoCorto("*periodo", $textos["PERIODO"], 2, 2, $datos->periodo, array("title" => $textos["AYUDA_PERIODO"],"onKeyPress" => "return campoEntero(event)"))
	    ),
	    array(
            HTML::campoTextoCorto("orden", $textos["ORDEN"], 15, 15, $datos->orden, array("title" => $textos["AYUDA_ORDEN"]))
	    ),
	    array(
            HTML::campoOculto("llave_principal", $url_id)
	    )
	);
...
/*** Validar los datos provenientes del formulario ***/
    ...
    /*** Validar codigo ***/
     if ($url_item == "codigo") {
        $existe = SQL::existeItem("plazos_pago_proveedores", "codigo", $url_valor,"codigo !=0 AND codigo != $url_id");
        if ($existe) {
    ...
    /*** Validar nombre ***/
     if ($url_item == "nombre") {
        $existe = SQL::existeItem("plazos_pago_proveedores", "nombre", $url_valor,"nombre !='' AND id != $url_id");
        if ($existe) {
...
/*** Modificar el elemento seleccionado ***/
    ...
    if(empty($forma_codigo)){
    ...
    }elseif($existe = SQL::existeItem("plazos_pago_proveedores", "nombre", $forma_nombre,"nombre !='' AND codigo != '$forma_llave_principal'")){
	    $error   = true;
        $mensaje = $textos["ERROR_EXISTE_NOMBRE"];

    }elseif($existe = SQL::existeItem("plazos_pago_proveedores", "nombre", $forma_codigo,"codigo !='' AND codigo != '$forma_llave_principal'")){
	    $error   = true;
        ...
        $consulta = SQL::modificar("plazos_pago_proveedores", $datos, "codigo = '$forma_llave_principal'");
