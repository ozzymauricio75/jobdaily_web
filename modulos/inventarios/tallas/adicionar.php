...
if (!empty($url_generar)) {
    ...
    $codigo = SQL::obtenerValor("tallas","MAX(codigo)","");
    if ($codigo){
        $codigo++;
    } else {
        $codigo = 1;
    }
    $orden = SQL::obtenerValor("tallas","MAX(orden)","");
    if ($orden){
        $orden++;
    } else {
        $orden = 1;
    ...
    $formularios["PESTANA_GENERAL"] = array(
        ...
        array(
            HTML::campoTextoCorto("*descripcion_corta", $textos["DESCRIPCION_CORTA"], 15, 15, "", array("title" => $textos["AYUDA_DESCRIPCION_CORTA"],"onBlur" => "validarItem(this);"))
        ),
        array(
...
/*** Validar los datos provenientes del formulario ***/
    ...
    /*** Validar codigo ***/
    if ($url_item == "codigo") {
        $existe = SQL::existeItem("tallas", "codigo", $url_valor,"codigo != '0'");
    ...
    if ($url_item == "descripcion_corta") {
        $existe = SQL::existeItem("tallas", "descripcion_corta", $url_valor,"descripcion_corta !=''");
...
/*** Adicionar los datos provenientes del formulario ***/
    ...
    if(empty($forma_codigo)){
    ...
    }elseif($existe = SQL::existeItem("tallas", "codigo", $forma_codigo)){
        $error   = true;
        $mensaje = $textos["ERROR_EXISTE_CODIGO"];

    }elseif($existe = SQL::existeItem("tallas", "descripcion_corta", $forma_descripcion_corta)){
        $error   = true;
        ...
        $insertar = SQL::insertar("tallas", $datos);
