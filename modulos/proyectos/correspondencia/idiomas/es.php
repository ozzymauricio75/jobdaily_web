<?php

/**
*
* Copyright (C) 2020 Jobdaily
* Raul Mauricio Oidor Lozano <ozzymauricio75@gmail.com>
*
* Este archivo es parte de:
* Jobdaily:: Sofware empresarial a la medida
*
* Este programa es software libre: usted puede redistribuirlo y/o
* modificarlo  bajo los términos de la Licencia Pública General GNU
* publicada por la Fundación para el Software Libre, ya sea la versión 3
* de la Licencia, o (a su elección) cualquier versión posterior.
*
* Este programa se distribuye con la esperanza de que sea útil, pero
* SIN GARANTÍA ALGUNA; ni siquiera la garantía implícita MERCANTIL o
* de APTITUD PARA UN PROPÓSITO DETERMINADO. Consulte los detalles de
* la Licencia Pública General GNU para obtener una información más
* detallada.
*
* Debería haber recibido una copia de la Licencia Pública General GNU
* junto a este programa. En caso contrario, consulte:
* <http://www.gnu.org/licenses/>.
*
**/

$textos = array(
    "GESTCORR"                          => "Correspondencia",
    "ADICCORR"                          => "Adicionar Correspondencia",
    "CONSCORR"                          => "Consultar Correspondencia",
    "MODICORR"                          => "Modificar Correspondencia",
    "ANULCORR"                          => "Anular Correspondencia",
    "ELIMCORR"                          => "Eliminar Correspondencia",
    "RECICORR"                          => "Recibir Correspondencia",
    "APROCORR"                          => "Aprobar para pagos",
    "REPOCORR"                          => "Reportes Correspondencia",
    "RECIBIR"                           => "Recibir",
    "REPORTE"                           => "Reportes",
    "NUMERO"                            => "Número",
    "VALOR"                             => "Valor",
    "IMPRIMIR_PDF"                      => "[Imprimir]",
    "CODIGO"                            => "Código",
    "TOTAL_ORDEN"                       => "Total orden",
    "TOTAL_CRUZADO"                     => "Total cruzado",
    "APLICA"                            => "Sin O.C",
    "TODOS"                             => "Todos",
    "FECHA_DOCUMENTO"                   => "Fecha documento",
    "VALOR_ORDEN_COMPRA"                => "Valor orden",
    "ESTADO_PDF"                        => "Estado",
    "VALOR_TOTAL"                       => "Valor total",
    "VALOR_DOCUMENTO"                   => "Valor documento",
    "BOTON_RECIBIR"                     => "",
    "PLANO_GENERADO"                    => "Archivo generado",
    "GENERAR_PLANO"                     => "Descargar archivo",
    "RECEPCIONADO"                      => "Recepcionado",
    "ENTREGADO"                         => "Entregado",
    "ANULADO"                           => "Anulado",
    "APROBAR"                           => "Aprobar",
    "ARCHIVO"                           => "Archivo",
    "PESTANA_REPORTE"                   => "Reportes",
    "PESTANA_DOCUMENTO"                 => "Documento",
    "ESTADO_CORRESPONDENCIA"            => "Estado",
    "RECIBIDO"                          => "Recibido",
    "ESTADO"                            => "Estado",
    "ESTADO_0"                          => "Recepcionado",
    "ESTADO_1"                          => "Entregado",
    "ESTADO_2"                          => "Anulado",
    "ESTADO_3"                          => "Aprobada",
    "ESTADO_RESIDENTE"                  => "Estado residente",
    "ESTADO_DIRECTOR"                   => "Estado director",
    "ESTADO_RESIDENTE_0"                => "No aprobado",
    "ESTADO_RESIDENTE_1"                => "Aprobado",
    "ESTADO_RESIDENTE_2"                => "Anulado",
    "ESTADO_DIRECTOR_0"                 => "No aprobado",
    "ESTADO_DIRECTOR_1"                 => "Aprobado",
    "PROYECTO"                          => "Proyecto",
    "NIT_PROVEEDOR"                     => "Nit",
    "FACTURA"                           => "Factura/Remisión",
    "CRUZADO"                           => "Cruzado con",
    "ORDEN_COMPRA"                      => "Orden compra",
    "RAZON_SOCIAL"                      => "Proveedor",
    "FECHA_RECEPCION"                   => "Fecha recepción",
    "FECHA_VENCIMIENTO"                 => "Fecha vencimiento",
    "FECHA_ENVIO"                       => "Fecha Recibido",
    "NO_ES_FACTURA"                     => " ",
    "OBSERVACIONES"                     => "Observaciones",
    "VALOR_DOCUMENTO"                   => "Valor documento",
    "VALOR"                             => "Valor",
    "RAZON_SOCIAL_PROVEEDOR"            => "Razón social proveedor",
    "TIPO_DOCUMENTO"                    => "Tipo de documento",
    "TIPO_DOCUMENTO_CRUCE"              => "Tipo documento a cruzar",
    "DOCUMENTO_CRUCE"                   => "Documento a cruzar",
    "DOCUMENTO_SOPORTE"                 => "Número Documento",
    "VALOR_PROYECTO"                    => "Valor proyecto",
    "EMPRESA"                           => "Empresa",
    "CONSORCIO"                         => "Consorcio o Unión temporal",
    "TIPO"                              => "Tipo",
    "TEXTO_CONSORCIO"                   => "Consorcio",
    "TEXTO_UNION_TEMPORAL"              => "Unión temporal",
    "NOMBRE"                            => "Nombre",
    "NOMBRE_CORTO"                      => "Nombre corto",
    "MUNICIPIO"                         => "Municipio",
    "DIRECCION_RESIDENCIA"              => "Direccion residencia",
    "DIRECCION_PROYECTO"                => "Direccion proyecto",
    "TELEFONO_1"                        => "Telefono 1",
    "TELEFONO_2"                        => "Telefono 2",
    "CELULAR"                           => "Celular",
    "FECHA_CIERRE"                      => "Fecha cierre",
    "EMPRESA_CONSOLIDA"                 => "Empresa consolida",
    "ALMACEN_CONSOLIDA"                 => "Consorcio consolida",
    "TIPO_EMPRESA"                      => "Tipo empresa",
    "TIPO_NEGOCIO"                      => "Tipo negocio",
    "TIPO"                              => "Tipo",
    "UNION_TEMPORAL"                    => "Union temporal",
    "PRINCIPAL"                         => "Principal",
    "ORDEN"                             => "Orden",
    "MANEJA_KARDEX"                     => "Maneja kardex",
    "TERCERO"                           => "Tercero",
    "NOMBRE_COMPLETO"                   => "Tercero",
    "ESTADO_ACTIVA"                     => "Abierto",
    "ESTADO_INACTIVA"                   => "Cerrado",
    "EMPRESA_DISTRIBUIDORA_MAYORISTA"   => "Distribuidora mayorista",
    "EMPRESA_VENTAS_PUBLICO"            => "Ventas publico",
    "EMPRESA_AMBAS"                     => "Ambas",
    "EMPRESA_SOPORTE"                   => "Empresa soporte",
    "VALOR_PROYECTO"                    => "Valor proyecto",
    "INDICADOR_NO"                      => "No",
    "INDICADOR_SI"                      => "Si",
    "ITEM"                              => "Item",
    "ITEM_CRUZADO"                      => "El documento ha sido cruzado.",
    "ITEM_ANULADO"                      => "El documento ha sido anulado.",
    "ITEM_APROBADO"                     => "El documento ha sido aprobado.",
    "ITEM_RECIBIDO"                     => "El documento ha sido recibido.",
    "ITEM_ELIMINADO"                    => "El documento ha sido eliminado.",
    "AYUDA_ESTADO_CORRESPONDENCIA"      => "Estado en que se encuentra la correspondencia",
    "AYUDA_PROYECTO"                    => "Seleccione un proyecto",
    "AYUDA_ORDEN_COMPRA"                => "Seleccione la orden de compra que corresponde al proyecto",
    "AYUDA_OBSERVACIONES"               => "Digite las observaciones",
    "AYUDA_FECHA_RECEPCION"             => "Seleccione la Fecha recepción",
    "AYUDA_FECHA_VENCIMIENTO"           => "Seleccione la Fecha vencimiento",
    "AYUDA_FECHA_ENVIO"                 => "Seleccione la Fecha envio",
    "AYUDA_NIT_PROVEEDOR"               => "Digite Nit proveedor",
    "AYUDA_VALOR_DOCUMENTO"             => "Digite el valor del documento",
    "AYUDA_DOCUMENTO_SOPORTE"           => "Digite el numero del documento que entrega el proveedor",
    "AYUDA_TIPO_DOCUMENTO"              => "Seleccione el tipo de documento",
    "AYUDA_DOCUMENTO_CRUCE"             => "Seleccione el documento a cruzar",
    "RAZON_SOCIAL_PROVEEDOR"            => "Razón social proveedor",
    "AYUDA_VALOR_PROYECTO"              => "Digite el valor del proyecto",
    "AYUDA_DIRECCION"                   => "Direccion donde se encuentra el consorcio",
    "AYUDA_CODIGO"                      => "Codigo del consorcio",
    "AYUDA_APLICA"                      => "Seleccione si aplica orden de compra",
    "AYUDA_ACTIVO"                      => "Estado actual del consorcio activo-inactivo",
    "AYUDA_EMPRESAS"                    => "Seleccione la empresa",
    "AYUDA_NOMBRE"                      => "Nombre del proyecto",
    "AYUDA_NOMBRE_CORTO"                => "Nombre corto del consorcio",
    "AYUDA_MUNICIPIOS"                  => "Municipio donde se encuentra el consorcio",
    "AYUDA_TELEFONO_1"                  => "Telefono principal",
    "AYUDA_TELEFONO_2"                  => "Telefono secundario",
    "AYUDA_CELULAR"                     => "Numero del celular",
    "AYUDA_FECHA_CIERRE"                => "Fecha de cierre del consorcio",
    "AYUDA_EMPRESA_CONSOLIDA"           => "Empresa en la cual se consolida el consorcio",
    "AYUDA_ALMACEN_CONSOLIDA"           => "Consorcio en el cual se consolida",
    "AYUDA_CONSORCIO"                   => "Consorcio o unión temporal que desarrolla el proyecto",
    "AYUDA_TIPO_EMPRESA"                => "Tipo de empresa al cual pertenece el consorcio",
    "AYUDA_TIPO_NEGOCIO"                => "Tipo de negocio al cual pertenece el consorcio",
    "AYUDA_MANEJA_KARDEX"               => "Almacén maneja kardex de inventarios",
    "AYUDA_ORDEN"                       => "Orden para los listados",
    "PESTANA_GENERAL"                   => "General",
    "PESTANA_CONTABLE"                  => "Contabilidad",
    "PESTANA_UBICACION"                 => "Ubicación",
    "ERROR_CRUZAR_ITEM"                 => "No se pudo cruzar el documento",
    "ERROR_ANULAR_ITEM"                 => "No se pudo anular el documento",
    "ERROR_ANULAR_ITEM"                 => "No se pudo anular el documento",
    "ERROR_AUTORIZADOS"                 => "No se puede modificar este documento porque ya se encuentra aprobado o autorizado",
    "ERROR_APROBAR_ITEM"                => "No se pudo aprobar el documento",
    "ERROR_RECIBIR_ITEM"                => "No se pudo recibir el documento",
    "ERROR_ELIMINAR_ITEM"               => "No se pudo eliminar el documento",
    "ERROR_ORDEN_ESTADO"                => "La operación no puede realizar debido a que el documento no está aprobado o está anulado.",
    "ERROR_ORDEN_ESTADO_AUTORIZADO"     => "La operación no puede realizar debido a que el documento No esta recibido.",
    "ERROR_EXISTE_CODIGO"               => "Ya existe un almacén con ese código",
    "ERROR_EXISTE_ORDEN"                => "No existe una orden de compra asociada al proyecto o al proveedor",
    "ERROR_EXISTE_NOMBRE"               => "Ya existe un almacén con ese nombre",
    "ERROR_EXISTE_NOMBRE_CORTO"         => "Ya existe un almacén con ese nombre corto",
    "ERROR_EXISTE_DIRECCION_RESIDENCIA" => "Ya existe un almacén con esa dirección",
    "ERROR_EXISTE_TELEFONO_1"           => "Ya existe un almacén con ese numero telefónico",
    "ERROR_FORMATO_CODIGO"              => "El codigo debe contener solo numeros",
    "REALIZA_ORDEN_COMPRA"              => "Autorizado para realizar ordenes de compra",
    "MANEJA_INVENTARIOS_MERCANCIA"      => "Maneja inventarios de mercancía",
    "CARTERA_CLIENTES_MAYORISTAS"       => "Maneja cartera de clientes mayoristas",
    "CARTERA_CLIENTES_DETALLISTAS"      => "Maneja cartera de clientes detallistas",
    "CUENTAS_PAGAR_PROVEEDORES"         => "Manejo de cuentas por pagar a proveedores",
    "NOMINA"                            => "Manejo de nómina",
    "CONTABILIDAD"                      => "Manejo contable",
    "ORDEN"                             => "Orden",
    "ORDEN_VACIO"                       => "Debe seleccionar el numero de orden de compra",
    "PROYECTO_VACIO"                    => "Debe ingresar el proyecto asociado al documento",
    "PROVEEDOR_VACIO"                   => "Debe ingresar un proveedor",
    "NOMBRE_DOCUMENTO_VACIO"            => "Debe ingresar un nombre para el archivo",
    "TIPO_DOCUMENTO_VACIO"              => "Debe ingresar el tipo del documento",
    "DOCUMENTO_SOPORTE_VACIO"           => "Debe ingresar el documento soporte del proveedor",
    "VALOR_VACIO"                       => "Debe ingresar el valor del documento",
    "FECHA_RECEPCION_VACIO"             => "Debe ingresar la fecha de recepcion",
    "FECHA_VENCIMIENTO_VACIO"           => "Debe ingresar la fecha de vencimiento",
    "FECHA_ENVIO_VACIO"                 => "Debe ingresar la fecha de envio",
    "CODIGO_SUCURSAL_VACIO"             => "Debe ingresar el código del consorcio que consolida",
    "CODIGO_EMPRESA_VACIO"              => "Debe ingresar el codigo de la empresa que consolida",
    "CREAR_PROYECTOS"                   => "No existen proyectos, debe crear por lo menos uno.",
    "ARCHIVO_DOCUMENTO"                 => "Documento",
    "AYUDA_ARCHIVO_DOCUMENTO"           => "Seleccione el documento",
    "NOMBRE_DOCUMENTO"                  => "Nombre documento ",
    "AYUDA_NOMBRE_DOCUMENTO"            => "Titulo para el documento"
);
/*$textos = array(
    "GESTSUCU"                          => "Almacenes",
    "ADICSUCU"                          => "Adicionar almacenes",
    "CONSSUCU"                          => "Consultar almacenes",
    "MODISUCU"                          => "Modificar almacenes",
    "ELIMSUCU"                          => "Eliminar almacenes",
    "CODIGO"                            => "Código",
    "SUCURSAL"                          => "Almacén",
    "EMPRESA"                           => "Empresa",
    "NOMBRE"                            => "Nombre",
    "NOMBRE_CORTO"                      => "Nombre corto",
    "MUNICIPIO"                         => "Municipio",
    "DIRECCION_RESIDENCIA"              => "Direccion residencia",
    "TELEFONO_1"                        => "Telefono 1",
    "TELEFONO_2"                        => "Telefono 2",
    "CELULAR"                           => "Celular",
    "FECHA_CIERRE"                      => "Fecha cierre",
    "ESTADO"                            => "Estado",
    "EMPRESA_CONSOLIDA"                 => "Empresa consolida",
    "ALMACEN_CONSOLIDA"                 => "Almacen consolida",
    "TIPO_EMPRESA"                      => "Tipo empresa",
    "TIPO_NEGOCIO"                      => "Tipo negocio",
    "ORDEN"                             => "Orden",
    "MANEJA_KARDEX"                     => "Maneja kardex",
    "TERCERO"                           => "Tercero",
    "NOMBRE_COMPLETO"                   => "Tercero",
    "ESTADO_ACTIVA"                     => "Activa",
    "ESTADO_INACTIVA"                   => "Inactiva",
    "EMPRESA_DISTRIBUIDORA_MAYORISTA"   => "Distribuidora mayorista",
    "EMPRESA_VENTAS_PUBLICO"            => "Ventas publico",
    "EMPRESA_AMBAS"                     => "Ambas",
    "EMPRESA_SOPORTE"                   => "Empresa soporte",
    "INDICADOR_NO"                      => "No",
    "INDICADOR_SI"                      => "Si",
    "AYUDA_DIRECCION"                   => "Direccion donde se encuentra el almacén",
    "AYUDA_CODIGO"                      => "Codigo de la empresa",
    "AYUDA_ACTIVO"                      => "Estado actual del almacen activo-inactivo",
    "AYUDA_EMPRESAS"                    => "Empresa a la que pertenece el almacén",
    "AYUDA_NOMBRE"                      => "Nombre del almacen",
    "AYUDA_NOMBRE_CORTO"                => "Nombre corto del almacen",
    "AYUDA_MUNICIPIOS"                  => "Municipio donde se encuentra el almacén",
    "AYUDA_TELEFONO_1"                  => "Telefono principal",
    "AYUDA_TELEFONO_2"                  => "Telefono secundario",
    "AYUDA_CELULAR"                     => "Numero del celular",
    "AYUDA_FECHA_CIERRE"                => "Fecha de cierre del almacén",
    "AYUDA_EMPRESA_CONSOLIDA"           => "Empresa en la cual se consolida el almacén",
    "AYUDA_ALMACEN_CONSOLIDA"           => "Almacén en el cual se consolida",
    "AYUDA_TIPO_EMPRESA"                => "Tipo de empresa al cual pertenece al almacén",
    "AYUDA_TIPO_NEGOCIO"                => "Tipo de negocio al cual pertenece la empresa",
    "AYUDA_MANEJA_KARDEX"               => "Almacén maneja kardex de inventarios",
    "AYUDA_ORDEN"                       => "Orden para los listados",
    "PESTANA_GENERAL"                   => "General",
    "PESTANA_CONTABLE"                  => "Contabilidad",
    "PESTANA_UBICACION"                 => "Ubicación",
    "ERROR_EXISTE_CODIGO"               => "Ya existe un almacén con ese código",
    "ERROR_EXISTE_NOMBRE"               => "Ya existe un almacén con ese nombre",
    "ERROR_EXISTE_NOMBRE_CORTO"         => "Ya existe un almacén con ese nombre corto",
    "ERROR_EXISTE_DIRECCION_RESIDENCIA" => "Ya existe un almacén con esa dirección",
    "ERROR_EXISTE_TELEFONO_1"           => "Ya existe un almacén con ese numero telefónico",
    "ERROR_FORMATO_CODIGO"              => "El codigo debe contener solo numeros",
    "REALIZA_ORDEN_COMPRA"              => "Autorizado para realizar ordenes de compra",
    "MANEJA_INVENTARIOS_MERCANCIA"      => "Maneja inventarios de mercancía",
    "CARTERA_CLIENTES_MAYORISTAS"       => "Maneja cartera de clientes mayoristas",
    "CARTERA_CLIENTES_DETALLISTAS"      => "Maneja cartera de clientes detallistas",
    "CUENTAS_PAGAR_PROVEEDORES"         => "Manejo de cuentas por pagar a proveedores",
    "NOMINA"                            => "Manejo de nómina",
    "CONTABILIDAD"                      => "Manejo contable",
    "ORDEN"                             => "Orden",
    "CODIGO_VACIO"                      => "Debe ingresar el código",
    "EMPRESA_VACIO"                     => "Debe seleccionar una empresa",
    "NOMBRE_VACIO"                      => "Debe ingresar el nombre para el almacen",
    "MUNICIPIO_VACIO"                   => "Debe ingresar el municipio",
    "DIRECCION_VACIO"                   => "Debe ingresar la dirección",
    "TELEFONO_VACIO"                    => "Debe ingresar el numero telefonico",
    "CODIGO_SUCURSAL_VACIO"             => "Debe ingresar el código del almacen que consolida",
    "CODIGO_EMPRESA_VACIO"              => "Debe ingresar el codigo de la empresa que consolida",
    "CREAR_EMPRESAS"                    => "No existen empresas, debe crear por lo menos una"
);*/
?>
