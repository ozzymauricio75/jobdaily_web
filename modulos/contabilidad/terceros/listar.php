...
if (!empty($url_generar)) {
    ...
    $formularios["PESTANA_GENERAL"] = array(
        array(
            HTML::listaSeleccionSimple("tipo_persona", $textos["TIPO_PERSONA"], $tipos_personas, ""),
            HTML::listaSeleccionSimple("tipo_documento", $textos["TIPO_DOCUMENTO_IDENTIDAD"], $tipos_documentos, "")
        ),
        array(
...
// Exportar los datos
    ...
    if($forma_tipo_persona != "0"){
        $condicion_tipo_persona = "tipo_persona = '".$forma_tipo_persona."'";
    }
    if($forma_tipo_documento != "0"){
        $condicion_tipo_documento = " AND codigo_tipo_documento = '".$forma_tipo_documento."'";
    }
    if($forma_tipo_documento != "0"){
        $condicion_tipo_documento = " AND codigo_tipo_documento = '".$forma_tipo_documento."'";
    }
    if($forma_codigo_localidad_residencia != ""){
        ...
        $condicion_localidad_residencia = " AND codigo_iso_localidad = '".$localidad_residencia[0]."'
                                            AND codigo_dane_departamento_localidad = '".$localidad_residencia[1]."'
                                            AND codigo_dane_municipio_localidad = '".$localidad_residencia[2]."'
                                            AND tipo_localidad = '".$localidad_residencia[3]."'
                                            AND codigo_dane_localidad = '".$localidad_residencia[4]."'";
    }
    if($forma_fechas != ""){
        $fechas        = explode("-",$forma_fechas);
        $fecha_inicial = trim($fechas[0]);
        $fecha_final   = trim($fechas[1]);
        $condicion_fechas = " AND (fecha_nacimiento BETWEEN '".$fecha_inicial."' AND '".$fecha_final."')";
    }
    ...
    if ($tipo_listado == 1){
        ...
        $archivo->textoTitulo    = $textos["REPORTE_TERCEROS"];
        $archivo->AddPage();
        $archivo->textoPiePagina = "";

        $archivo->SetFont('Arial','B',6);
        $tituloColumnas = array(
            $textos["TIPO_DOCUMENTO"], $textos["DOCUMENTO_IDENTIDAD"], $textos["TIPO_PERSONA"], $textos["NOMBRE_COMPLETO"], $textos["LUGAR_RESIDENCIA"], $textos["TELEFONO"], $textos["CELULAR"], $textos["CORREO"], $textos["GENERO"], $textos["FECHA_NACIMIENTO"]
        );
        $anchoColumnas = array(30,30,30,60,60,30,20,35,15,25);
        $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
        $archivo->Ln(4);
    } else {
        $archivo = fopen($nombreArchivo,"a+");
        $titulos_plano = "\"".$textos["TIPO_DOCUMENTO"]."\";\"".$textos["DOCUMENTO_IDENTIDAD"]."\";\"".$textos["TIPO_PERSONA"]."\";\"".$textos["NOMBRE_COMPLETO"]."\";\"".$textos["LUGAR_RESIDENCIA"]."\";\"".$textos["TELEFONO"]."\";\"".$textos["CELULAR"]."\";\"".$textos["CORREO"]."\";\"".$textos["GENERO"]."\";\"".$textos["FECHA_NACIMIENTO"]."\"\n";
        fwrite($archivo, $titulos_plano);
    }

    $consulta = SQL::seleccionar(array("terceros"),array("*"),$condicion." AND documento_identidad != 0","","codigo_tipo_documento,documento_identidad,tipo_persona,primer_nombre,razon_social");
    ...
    if(SQL::filasDevueltas($consulta)){
        ...
        while($datos = SQL::filaEnObjeto($consulta)){
            ...
            $tipo_persona    = $tipos_personas[$datos->tipo_persona];
            ...
            if($tipo_listado == 1){
                ...
                $archivo->Cell(30, 4, $tipo_persona, 0, 0, "L");
                ...
                $contenido = "\"$tipo_documento\";\"$datos->documento_identidad\";\"$tipo_persona\";\"$tercero\";\"$localidad\";\"$datos->telefono_principal\";\"$datos->celular\";\"$datos->correo\";\"$datos->genero\";\"$datos->fecha_nacimiento\"\n";
