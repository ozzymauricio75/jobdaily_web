...
if(!empty($url_generar)){
    ...
    if(empty($url_id)){
        ...
        $plan_contable  = SQL::obtenerValor("seleccion_plan_contable_flujo_bancos", "SUBSTRING_INDEX(codigo_contable,'|',1)", "id = '".$datos->codigo_plan_contable."'");
        ...
        if($datos->codigo_auxiliar_contable != 0){
            ...
            $auxiliar           = array(HTML::mostrarDato("auxiliar_contable", $textos["AUXILIAR_CONTABLE"], $auxiliar_contable));
        }else{
            $auxiliar = array(HTML::campoOculto("auxiliar_contable", $datos->codigo_auxiliar_contable));
        }
