...
// Exportar los datos
    ...
    if ($tipo_listado == 1){
        ...
        $archivo->textoTitulo    = $textos["REPORTE_AEDIAN"];
        $archivo->textoPiePagina = "";
        $archivo->AddPage();

        $archivo->SetFont('Arial','B',6);
        $tituloColumnas = array($textos["CODIGO_DIAN"],$textos["DESCRIPCION"]);
        $anchoColumnas  = array(30,100);
        $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
        $archivo->Ln(4);
    } else {
        $archivo       = fopen($nombreArchivo,"a+");
        $titulos_plano = "\"".$textos["CODIGO_DIAN"]."\";\"".$textos["DESCRIPCION"]."\"\n";
        fwrite($archivo, $titulos_plano);
    }

    $consulta = SQL::seleccionar(array("actividades_economicas_dian"),array("*"),$condicion);
    ...
    if(SQL::filasDevueltas($consulta)){
        ...
        while($datos = SQL::filaEnObjeto($consulta)){
            ...
            if($tipo_listado == 1){
                ...
                $archivo->Cell(30, 4, $datos->codigo_dian, 0, 0, "C",true,"",true);
                $archivo->Cell(100, 4, $datos->descripcion, 0, 0, "L",true,"",true);
                $archivo->Ln(4);
            }else{
                $datos->descripcion = str_replace(";","",$datos->descripcion);
                $contenido = "\"".$datos->codigo_dian."\";\"".$datos->descripcion."\"\n";
