...
// Exportar los datos
    ...
    $condicion = "codigo_iso != ''";

    //=============== INICIO CONDICION ===============//
    /*if($forma_ != ""){
        $condicion .= " AND campo = '".$forma_."'";
    }*/
    //================ FIN  CONDICION ================//
    ...
    if ($tipo_listado == 1){
        ...
        $archivo->textoTitulo    = $textos["LISTPAIS"];
        ...
        $tituloColumnas = array($textos["CODIGO_ISO"],$textos["CODIGO_INTERNO"],$textos["NOMBRE"]);
        $anchoColumnas  = array(25,25,100);
        $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
        $archivo->Ln(4);
    } else {
        $archivo       = fopen($nombreArchivo,"a+");
        $titulos_plano = "\"".$textos["CODIGO_ISO"]."\";\"".$textos["CODIGO_INTERNO"]."\";\"".$textos["NOMBRE"]."\"\n";
        fwrite($archivo, $titulos_plano);
    }

    $consulta = SQL::seleccionar(array("paises"),array("*"),$condicion);
    ...
    if(SQL::filasDevueltas($consulta)){
        ...
        while($datos = SQL::filaEnObjeto($consulta)){
            ...
            if($tipo_listado == 1){
                ...
                $archivo->Cell(25, 4, $datos->codigo_iso, 1, 0, "L",true,"",true);
                $archivo->Cell(25, 4, $datos->codigo_interno, 1, 0, "L",true,"",true);
                $archivo->Cell(100, 4, $datos->nombre, 1, 0, "L",true,"",true);
                $archivo->Ln(4);
            }else{

                $pais = str_replace(";","",$datos->nombre);

                $contenido = "\"".$datos->codigo_iso."\";\"".$datos->codigo_interno."\";\"".$pais."\"\n";
