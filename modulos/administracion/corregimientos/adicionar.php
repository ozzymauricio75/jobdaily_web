...
if(isset($url_validarItemsllaves))
{
    ...
    if($url_item=="nombre" && !empty($url_valor))
    {
        ...
        $existe = SQL::existeItem("buscador_corregimientos","nombre",$url_valor_item,"id = '$llave_primaria'");
        if ($existe) {
    ...
    }elseif($url_item=="codigo_dane_localidad" && !empty($url_valor))
    {
         $llave_primaria=explode("|", $url_valor);
        $llave_primaria =$llave_primaria[0]."|".$llave_primaria[1]."|".$llave_primaria[2]."|C|".$llave_primaria[3]; //str_pad($llave_primaria[2],8,"0", STR_PAD_LEFT)
        $existe = SQL::existeItem("buscador_corregimientos","codigo_localidad",$url_valor_item,"id = '$llave_primaria'");
        if ($existe) {
    ...
    }elseif($url_item=="codigo_interno" && !empty($url_valor))
    {
        ...
        $existe = SQL::existeItem("localidades","codigo_interno",$url_valor_item,"codigo_iso='$llave_primaria[0]' AND codigo_dane_departamento='$llave_primaria[1]' AND codigo_dane_municipio='$llave_primaria[2]' AND tipo='C'");
        if ($existe) {
...
if (!empty($url_generar)) {
    ...
    $municipios    = HTML::generarDatosLista("municipios", "codigo_dane_municipio", "nombre", "codigo_dane_departamento = '".array_shift(array_keys($departamentos))."'");
    ...
    $formularios["PESTANA_GENERAL"] = array(
        ...
        array(
            HTML::campoTextoCorto("*codigo_dane_localidad", $textos["CODIGO_DANE"], 2, 2, "", array("title" => $textos["AYUDA_CODIGO_DANE"],"onBlur" => "validarItemsllaves(this,'$campo_llave_primaria')"))
        ),
        array(
            HTML::campoTextoCorto("codigo_interno", $textos["CODIGO_INTERNO"], 4, 4, "", array("title" => $textos["AYUDA_CODIGO_INTERNO"],"onBlur" => "validarItemsllaves(this,'$campo_llave_primaria')")),
        )
    );
...
/*** Validación en línea de los datos provenientes del formulario ***/
    ...
    $condicion = "codigo_iso = '$forma_pais' AND codigo_dane_departamento='$forma_departamento' AND codigo_dane_municipio = '$forma_municipio' AND tipo = 'C' AND codigo_dane_localidad='$forma_codigo_dane_localidad'";
    ...
    } elseif (!empty($forma_codigo_interno) && SQL::existeItem("localidades","codigo_interno",$forma_codigo_interno,"codigo_iso='$forma_pais' AND codigo_dane_departamento='$forma_departamento' AND codigo_dane_municipio='$forma_municipio' AND tipo='C'")) {
        $error   = true;
        $mensaje =  $textos["ERROR_EXISTE_CODIGO_INTERNO"];
