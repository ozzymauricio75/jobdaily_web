...
if(isset($url_validarItemsllaves))
{
    ...
    if($url_item=="nombre" && !empty($url_valor))
    {
        ...
        $llave_primaria =$llave_primaria[0]."|".$llave_primaria[1]."|".$llave_primaria[2]."|C|".$llave_primaria[3]; //str_pad($llave_primaria[2],8,"0", STR_PAD_LEFT)
        $existe = SQL::existeItem("buscador_corregimientos","nombre",$url_valor_item,"id != '$url_id' AND nombre!=''");
        if ($existe) {
    ...
    }elseif($url_item=="codigo_dane_localidad" && !empty($url_valor))
    {
         $llave_primaria=explode("|", $url_valor);
        $llave_primaria =$llave_primaria[0]."|".$llave_primaria[1]."|".$llave_primaria[2]."|C|".$llave_primaria[3]; //str_pad($llave_primaria[2],8,"0", STR_PAD_LEFT)
        $existe = SQL::existeItem("buscador_corregimientos","codigo_localidad",$url_valor_item,"id != '$url_id' AND codigo_localidad!=''");
        if ($existe) {
    ...
    }elseif($url_item=="codigo_interno" && !empty($url_valor))
    {
         $llave_primaria=explode("|", $url_valor);
        $llave_primaria =$llave_primaria[0]."|".$llave_primaria[1]."|".$llave_primaria[2]."|C|".$llave_primaria[3]; //str_pad($llave_primaria[2],8,"0", STR_PAD_LEFT)
        $existe = SQL::existeItem("buscador_corregimientos","codigo_interno",$url_valor_item,"id != '$url_id' codigo_interno!=0");
        if ($existe) {
...
if (!empty($url_generar)) {
    ...
    if (empty($url_id)) {
        ...
        $formularios["PESTANA_GENERAL"] = array(
            ...
            array(
                HTML::campoTextoCorto("*codigo_dane_localidad", $textos["CODIGO_DANE"], 2, 2, $datos->codigo_localidad, array("title" => $textos["AYUDA_CODIGO_DANE"],"onBlur" => "validarItemsllaves(this,'$campo_llave_primaria','$url_id')"))
            ),
            array(
                HTML::campoTextoCorto("codigo_interno", $textos["CODIGO_INTERNO"], 4, 4,(int)($datos->codigo_interno), array("title" => $textos["AYUDA_CODIGO_INTERNO"],"onBlur" => "validarItemsllaves(this,'$campo_llave_primaria','$url_id')", "onKeyPress" => "return campoEntero(event)")),
            )
        );
...
/*** Validación en línea de los datos provenientes del formulario ***/
    ...
    if (empty($forma_nombre)) {
    ...
    }  elseif (SQL::existeItem("buscador_corregimientos","nombre",$forma_nombre,"id != '$forma_id'")) {
        $error   = true;
        $mensaje =  $textos["ERROR_EXISTE_NOMBRE"];

    } elseif (!empty($forma_codigo_interno) && SQL::existeItem("buscador_corregimientos","codigo_interno",$forma_codigo_interno,"id != '$forma_id'")) {
        $error   = true;
        $mensaje =  $textos["ERROR_EXISTE_CODIGO_INTERNO"];

    } elseif (!empty( $forma_codigo_dane_localidad) && SQL::existeItem("buscador_corregimientos","codigo_localidad",$forma_codigo_dane_localidad,"id != '$forma_id'")) {
        $error   = true;
        $mensaje =  $textos["ERROR_EXISTE_CODIGO_DANE"];
        ...
        $insertar = SQL::modificar("localidades",$datos,"codigo_dane_localidad = '$codigo_dane_localidad ' AND codigo_iso='$codigo_iso'  AND codigo_dane_departamento='$codigo_dane_departamento' AND codigo_dane_municipio='$codigo_dane_municipio' AND tipo='$tipo'");
