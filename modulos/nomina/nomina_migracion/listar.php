...
if (!empty($url_generar)) {
    ...
    if (!isset($forma_sucursales)){
        ...
        } else if ($forma_orden__empleado=="2"){
            $orden_empleado = "nombre_empleado";
        } else {
            $orden_empleado = "documento_identidad_empleado ASC";
        }
        ...
        foreach ($forma_sucursales AS $codigo_sucursal){
            ...
            $condicion_planilla  = "codigo_sucursal = '$codigo_sucursal'";
            $condicion_planilla .= " AND fecha_pago_planilla BETWEEN '$fecha1' AND '$fecha2'";
            if(!empty($forma_documento_identidad)){
                $condicion_planilla .= " AND documento_identidad_empleado = '$forma_documento_identidad'";
            }

            $consulta_planilla = SQL::seleccionar(array("consulta_movimientos_nomina_migracion"),array("*"),$condicion_planilla,"","codigo_sucursal,$orden_empleado,fecha_pago_planilla ASC");
            $contabilizado = false;
            ...
            if (SQL::filasDevueltas($consulta_planilla)) {
                ...
                if ($forma_tipo_listado==1){
                    ...
                    $archivo->textoTitulo    = $textos["LISTNMIG"] . " ". $textos["FECHA"] . ": " . $forma_fechas;
                ...
                if ($forma_tipo_listado==1){
                    ...
                    $tituloColumnas = $textos["DOCUMENTO"].";".$textos["NOMBRE_EMPLEADO"].";".$textos["TRANSACCION_CONTABLE"].";".$textos["CUENTA_CONTABLE"].";".$textos["SENTIDO"].";".$textos["VALOR_MOVIMIENTO"].";".$textos["FECHA_REGISTRO"].";".$textos["FECHA_PAGO"].";".$textos["ANEXO"].";".$textos["AUXILIAR"]."\n";
                ...
                while ($datos = SQL::filaEnObjeto($consulta_planilla)) {
                    ...
                    if ($forma_tipo_listado==1){
                        ...
                        if ($datos->sentido=="D"){
                            $valor = "$ ".number_format($datos->valor_movimiento,0);
                        } else {
                            $valor = "($ ". number_format($datos->valor_movimiento,0) .")";
                        }
                        ...
                        $archivo->Cell(20, 5, $valor, 0, 0, "R", false,"",true);
                        ...
                        if ($datos->sentido=="D"){
                            $valor = (int)$datos->valor_movimiento;
                        } else {
                            $valor = (int)$datos->valor_movimiento * (-1);
                        }
                        $tituloColumnas = (int)$datos->documento_identidad_empleado.";$nombre_empleado_listado;$datos->codigo_transaccion_contable - $transaccion;$datos->codigo_contable - $cuenta;$datos->sentido;";
                        $tituloColumnas .= $valor.";$datos->fecha_registro;$datos->fecha_pago_planilla;$anexo;$auxiliar\n";
                        fwrite($archivo,$tituloColumnas);
                    }
                    if($datos->sentido == 'D'){
