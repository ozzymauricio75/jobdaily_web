...
{
    if(isset($url_fecha_generacion)){
       $condicion =  "documento_identidad_empleado='$url_documento_identidad' AND fecha_inicio='$url_fecha_generacion'";
    }else{
       $condicion =  "documento_identidad_empleado='$url_documento_identidad' AND fecha_inicio='$url_fecha_inicio' AND hora_inicio='$url_hora_inicio' AND hora_fin='$url_hora_fin'";
    }
    $eliminar = SQL::eliminar("movimiento_tiempos_laborados",$condicion);
    $respuesta = array();
    $respuesta[0] = $eliminar;
    if($eliminar){
        $respuesta[1] = $textos["HORA_BORRADA_EXITOSAMENTE"];
    }else{
        $respuesta[1] = $textos["ERROR_BORRAR_HORA"];
    }
...
{
    $fecha_inicio = SQL::obtenerValor("sucursal_contrato_empleados","fecha_ingreso","documento_identidad_empleado='$url_documento_identidad'", "", "fecha_ingreso DESC", 0, 1);
...
if(!empty($url_generar)){
    ...
    if(empty($url_id)){
        ...
        if(SQL::filasDevueltas($consulta_movimiento)){
            ...
            $fechas          = str_replace("-","/", $datos->fecha_inicio)." - ".str_replace("-","/",$datos->fecha_fin);
            // Determino mediante el turno a tormar, si el de la asignacion o el del contrato dependiendo la fecha actual
            $fecha_actual            = date("Y-m-d");
            $horas_generadas         = array();
            $consulta_turno_empleado = SQL::seleccionar(array("asignacion_turnos"),array("*")," ('$fecha_actual' BETWEEN fecha_inicial AND fecha_final) AND documento_identidad_empleado='$documento_identidad_empleado' ","","fecha_inicial DESC ",0,1);
            $datos_empleado          = SQL::filaEnObjeto($consulta_turno_empleado);
            if($datos_empleado){
                $consulta_turno   = SQL::seleccionar(array("turnos_laborales"),array("*"),"codigo='$datos_empleado->codigo_turno'");
                $datos_turno      = SQL::filaEnObjeto($consulta_turno);
                ...
                $consulta_turno_empleado = SQL::seleccionar(array("sucursal_contrato_empleados"),array("*"),"documento_identidad_empleado='$documento_identidad_empleado' ","","fecha_ingreso_sucursal DESC ",0,1);
                $datos_empleado          = SQL::filaEnObjeto($consulta_turno_empleado);
                $consulta_turno          = SQL::seleccionar(array("turnos_laborales"),array("*"),"codigo='$datos_empleado->codigo_turno_laboral'");
                $datos_turno             = SQL::filaEnObjeto($consulta_turno);
                $descripcion             = $datos_turno->descripcion;
            ...
            $estado_festivos= array($textos["NO"],$textos["SI"]);
            if($permite_festivos == "0"){
                $clase_contenedor_turno_laboral_1 = "";
                $permite_festivos_1               = $textos["NO"];
                $permite_festivos_2               = "";
                $clase_contenedor_turno_laboral_2 = "oculto";
                $paga_domingo                     = "";
                $paga_festivos                    = "";
            }else{
                $clase_contenedor_turno_laboral_1 = "oculto";
                $permite_festivos_1               = "";
                $permite_festivos_2               = $textos["SI"];
                $clase_contenedor_turno_laboral_2 = "";
                $paga_domingo                     = $estado_festivos[$paga_domingo];
                $paga_festivos                    = $estado_festivos[$paga_festivos];
            }
            ...
            if(SQL::filasDevueltas($consulta)){
                while($datos_movimientos = SQL::filaEnObjeto($consulta)){
                    $fecha_inicio = str_replace("/","-",$datos_movimientos->fecha_inicio);
                    $fecha_fin = str_replace("/","-", $datos_movimientos->fecha_fin);
                    ...
                    $boton             = HTML::boton("remover","","removerItemsModificar(this);", "eliminar");
                    $celda_oculta      = HTML::campoOculto("fecha_inicio[]",$fecha_inicio, array("class" => "fecha_inicio"));
                    $celda_oculta     .= HTML::campoOculto("hora_inicio[]",$hora_inicial, array("class" => "hora_inicio"));
                    $celda_oculta     .= HTML::campoOculto("fecha_fin[]",$fecha_fin , array("class" => "fecha_fin"));
                    $celda_oculta     .= HTML::campoOculto("hora_fin[]",$hora_fin, array("class" => "hora_fin"));
                    $celda_oculta     .= HTML::campoOculto("codigo_transaccion_tiempo[]",$datos_movimientos->codigo_transaccion_tiempo, array("class" => "codigo_transaccion_tiempo"));
                    $celda_oculta     .= HTML::campoOculto("codigo_contable[]",$datos_movimientos->codigo_contable, array("class" => "codigo_contable"));
                    $celda_oculta     .= HTML::campoOculto("minutos[]",$datos_movimientos->cantidad_minutos, array("class" => "minutos"));
                    $celda_oculta     .= HTML::campoOculto("horas[]",$datos_movimientos->cantidad_horas, array("class" => "horas"));

                    $preferencia_valor_horas_normales = SQL::obtenerValor("preferencias", "valor", "variable='horas_normales' AND tipo_preferencia = 1");
                    if($datos_movimientos->codigo_transaccion_tiempo==$preferencia_valor_horas_normales){
                        $celda_check = HTML::marcaChequeo("autorizarExtra","",1,true,array("disabled" => "disabled"));
                    }else{
                        $celda_check = HTML::marcaChequeo("autorizarExtra","",1,true);
                    }
            ...
            $formularios["PESTANA_BASICA"] = array(
                array(
                    HTML::mostrarDato("nombre_sucursal",$textos["SUCURSAL"],$sucursal),
                    ...
                    HTML::campoOculto("fecha_generacion",$fecha_inicio)
                ),
                array(
                    HTML::listaSeleccionSimple("*anexos_contables", $textos["ANEXO_CONTABLE"],$anexos_contables,"", array("title" => $textos["AYUDA_SUCURSAL"],"onchange" => "recargarLista('anexos_contables','auxiliares_contables');")),
                    HTML::listaSeleccionSimple("*auxiliares_contables", $textos["AUXILIAR_CONTABLE"],$listado_auxiliares, "", array("title" => $textos["AYUDA_SUCURSAL"],"onchange" => "")),

                ),
                array(
                    HTML::campoTextoCorto("*fechas", $textos["FECHA_INICIO"].'  -  '.$textos["FECHA_FIN"], 25, 25,$fechas, array("title" => $textos["AYUDA_FECHAS"],"class" => "fechaRango")),//,"onclick" => "obtenerRango();"
                    HTML::contenedor(HTML::campoTextoCorto("*turno_laboral_1", $textos["TURNO_LABORAL"],40, 25, $descripcion, array("title" => $textos["AYUDA_FECHAS"], "class" => "turno_laboral","disabled" => "disabled"))."<br/><br/>".HTML::contenedor(HTML::mostrarDato("permite_festivos_1",$textos["PEMITE_FESTIVOS"],$permite_festivos_1)),array("id" => "contenedor_turno_laboral_1","class" => $clase_contenedor_turno_laboral_1)),
                    HTML::contenedor(HTML::campoTextoCorto("*turno_laboral_2", $textos["TURNO_LABORAL"],40, 25, $descripcion, array("title" => $textos["AYUDA_FECHAS"], "class" => "turno_laboral","disabled" => "disabled"))."<br/><br/>".HTML::mostrarDato("permite_festivos_2",$textos["PEMITE_FESTIVOS"],$permite_festivos_2).HTML::mostrarDato("poga_dominical",$textos["PAGO_DOMINICAL"],$paga_domingo).HTML::mostrarDato("poga_festivo",$textos["PAGO_FESTIVOS"],$paga_festivos),array("id" => "contenedor_turno_laboral_2","class" => $clase_contenedor_turno_laboral_2)),
                    HTML::campoOculto("rango_fecha",""),
                ),
                array(
                    HTML::mostrarDato("mostrar",$textos["AUTORIZA_EXTRAS"],""),
                    HTML::marcaChequeo("autorizarExtra","",1,true,array("class" => "autorizarExtra")),
                ),
                array(
                ...
                array(
                    ...
                    HTML::boton("botonEliminar",$textos["ELIMINAR_TODOS"],"removerTableModificar();", "eliminar"),
            ...
            $botones   = array(HTML::boton("botonAceptar", $textos["ACEPTAR"], "horasAutorizadasExtras();modificarItem('$url_id');", "aceptar"));
