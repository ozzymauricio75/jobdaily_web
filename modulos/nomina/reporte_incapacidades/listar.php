...
// Exportar los datos
    ...
    if (!isset($forma_sucursales)){
        ...
        foreach ($forma_sucursales AS $codigo_sucursal){
            ...
            $condicion = "codigo_sucursal ='$codigo_sucursal' AND (fecha_inicial_incapacidad BETWEEN '$fecha1' AND '$fecha2')";
            ...
            if ($forma_tipo_listado=="1"){
                $archivo->AddPage();
                $tituloColumnas = array($textos["SUCURSAL"],$textos["CEDULA"],$textos["EMPLEADO"],$textos["FECHA_INICIAL"],$textos["FECHA_FINAL"],$textos["TIPO_TRANSACCION"],$textos["ID_MOTIVO_INCAPACIDAD"],$textos["NO_DIAS"],$textos["VALOR_MOVIMIENTO"]);
                $anchoColumnas  = array(25,15,40,17,17,30,30,12,25);
                $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
                $archivo->Ln(4);
            } else {
                $tituloColumnas = $textos["SUCURSAL"].";".$textos["CEDULA"].";".$textos["EMPLEADO"].";".$textos["FECHA_INICIAL"].";".$textos["FECHA_FINAL"].";".$textos["TIPO_TRANSACCION"].";".$textos["ID_MOTIVO_INCAPACIDAD"].";".$textos["NO_DIAS"].";".$textos["VALOR_MOVIMIENTO"]."\n";
                fwrite($archivo,$tituloColumnas);
            }
            $i = 0;
            $agrupamiento = "documento_identidad_empleado,codigo_transaccion_tiempo,fecha_inicial_incapacidad";
            $ordenamiento = "codigo_sucursal,$orden_empleado,fecha_incapacidad";
            $consulta = SQL::seleccionar(array("consulta_reporte_incapacidades"), array("*"),$condicion,$agrupamiento,$ordenamiento);

            if (SQL::filasDevueltas($consulta)) {
                $documento_identidad_anterior = 0;
                $total_empleado = 0;
                $dias_empleado  = 0;
                $total_sucursal = 0;
                ...
                while($datos = SQL::filaEnObjeto($consulta)) {
                    ...
                    $condicion .= " AND fecha_inicial_incapacidad = '".$datos->fecha_inicial_incapacidad."'";

                    $consulta2 = SQL::seleccionar(array("reporte_incapacidades"), array("*"),$condicion);
                    $cantidad = SQL::filasDevueltas($consulta2);

                    $fecha_comienzo = strtotime($datos->fecha_incapacidad);
                    ...
                    $motivo      = SQL::obtenerValor("motivos_incapacidad","descripcion"," codigo = '".$datos->codigo_motivo_incapacidad."'");
                    ...
                    if ($forma_tipo_listado==1){
                        ...
                        if ($documento_identidad_anterior!= $datos->documento_identidad_empleado && $documento_identidad_anterior!="" && empty($forma_documento_identidad)){
                            ...
                            $archivo->Cell(174, 4, $textos["TOTAL_EMPLEADO"], 1, 0, "R", true,"",true);
                            $archivo->Cell(12, 4, $dias_empleado, 1, 0, "R", true,"",true);
                            $archivo->Cell(25, 4, "$ ".number_format($total_empleado,0), 1, 0, "R", true,"",true);
                            $archivo->Ln(4);
                            $total_empleado = 0;
                            $dias_empleado  = 0;
                        ...
                        $archivo->Cell(17, 4, $datos->fecha_incapacidad, 1, 0, "C", true,"",true);
                        ...
                        $archivo->Cell(25, 4, "$ ".number_format($datos->valor_movimiento * $cantidad,0), 1, 0, "R", true,"",true);
                        $archivo->Ln(4);
                        $dias_empleado  += $cantidad;
                        $total_empleado += (int)($datos->valor_movimiento * $cantidad);

                        $dias_sucursal  += $cantidad;
                        $total_sucursal += (int)($datos->valor_movimiento * $cantidad);

                        $documento_identidad_anterior = $datos->documento_identidad_empleado;
                    } else {
                        $tituloColumnas = $sucursal.";".$datos->documento_identidad_empleado.";".$empleado.";".$datos->fecha_incapacidad.";".$fecha_fin.";".$transaccion.";".$motivo.";".$cantidad.";$ ".number_format($datos->valor_movimiento * $cantidad,0)."\n";
                        fwrite($archivo,$tituloColumnas);
                    }
                    $i++;
                }
            ...
            if ($forma_tipo_listado==1 && $i>0){
                ...
                $archivo->SetFont('Arial',"B",6);
                $archivo->Cell(174, 4, $textos["TOTAL_SUCURSAL"], 1, 0, "R", true,"",true);
                $archivo->Cell(12, 4, $dias_sucursal, 1, 0, "R", true,"",true);
                $archivo->Cell(25, 4, "$ ".number_format($total_sucursal,0), 1, 0, "R", true,"",true);
                $archivo->Ln(4);
