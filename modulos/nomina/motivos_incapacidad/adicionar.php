...
if (!empty($url_generar)) {
    ...
    $consecutivo = (int)SQL::obtenerValor("motivos_incapacidad","max(codigo)","");
    if($consecutivo){
        $consecutivo++;
    }else{
        $consecutivo=1;
    ...
    if($url_item == "codigo"){
        $existe = SQL::existeItem("motivos_incapacidad","codigo",$url_valor,"codigo != 0");
        if($existe){
            HTTP::enviarJSON($textos["ERROR_EXISTE_MOTIVO"]);
        }
    }else if($url_item == "descripcion"){
        $existe = SQL::existeItem("motivos_incapacidad","descripcion",$url_valor,"codigo != 0");
        if($existe){
    ...
    if(empty($forma_codigo) || $codigo == 0){
    ...
    } else if(SQL::existeItem("motivos_incapacidad","codigo",$forma_codigo,"codigo != 0")){
        $error = true;
        $mensaje = $textos["ERROR_EXISTE_MOTIVO"];
    } else if(SQL::existeItem("motivos_incapacidad","descripcion",$forma_descripcion,"codigo != 0")){
        $error = true;
        ...
        $insertar = SQL::insertar("motivos_incapacidad", $datos);
