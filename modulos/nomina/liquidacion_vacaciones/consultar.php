        ...
        if($continuar){
            ...
            $formularios["PESTANA_BASICA"] = array(
                array(
                    ...
                    HTML::mostrarDato("estado_liquidacion",$textos["ESTADO"],$estado_liquidacion[$datos_liquidacion->estado_liquidacion])
                ),
                array(
            ...
            /////////////Cargo los datos si fue pagada////////////////
            if($datos_liquidacion->estado_liquidacion=="4"){
                //echo var_dump($condicion);
                $consulta_pago_liquidacion = SQL::seleccionar(array("pago_liquidaciones_vacaciones"),array("*"),$condicion);
                $datos_pago_liquidacion    = SQL::filaEnObjeto($consulta_pago_liquidacion);

                $nombre_sucursal       = SQL::obtenerValor("sucursales","nombre","codigo='$datos_pago_liquidacion->codigo_sucursal'");
                $nombre_tipo_documento = SQL::obtenerValor("tipos_documentos","descripcion","codigo='$datos_pago_liquidacion->codigo_tipo_documento'");
                $decripcion_cuenta     = SQL::obtenerValor("seleccion_plan_contable","SUBSTRING_INDEX(codigo_contable,'|',1)","id='$datos_pago_liquidacion->codigo_contable'");

                $formularios["FORMA_PAGO"] = array(
                    array(
                        HTML::mostrarDato("codigo_sucursal",$textos["SUCURSAL_PAGA"],$nombre_sucursal),
                        HTML::mostrarDato("tipo_documento",$textos["TIPO_DOCUMENTO"],$nombre_tipo_documento),
                        HTML::mostrarDato("consecutivo_documento",$textos["CONSECUTIVO_DOCUMENTO"],(int)$datos_pago_liquidacion->consecutivo_documento)
                    ),array(
                        HTML::mostrarDato("cuenta",$textos["CUENTA_BANCARIA"],$decripcion_cuenta),
                    )
                );
                $genera_cheques = SQL::obtenerValor("tipos_documentos", "genera_cheque", "codigo = '$datos_pago_liquidacion->codigo_tipo_documento'");
                
                if($genera_cheques=='1'){
                    $nombre_banco = SQL::obtenerValor("bancos","descripcion","codigo='$datos_pago_liquidacion->codigo_banco'");
                    $descripcion_cuenta_bancaria = $nombre_banco." - No. ".$datos_pago_liquidacion->numero;

                    $contenido[] = array(
                        HTML::mostrarDato("cuenta_bancaria",$textos["CUENTA_BANCARIA"],$descripcion_cuenta_bancaria),
                        HTML::mostrarDato("cosecutivo_cheque",$textos["CONSECUTIVO_CHEQUE"],(int)$datos_pago_liquidacion->consecutivo_cheque),
                    );
                    $formularios["FORMA_PAGO"] = array_merge($formularios["FORMA_PAGO"], $contenido);
                }
            }
            /////////////////////////////////////////////////////////
