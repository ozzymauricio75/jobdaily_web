...
if (!empty($url_generar)) {
    ...
    $formularios["PESTANA_GENERAL"] = array(
        array(
            HTML::listaSeleccionSimple("*codigo_departamento", $textos["CODIGO_DEPARTAMENTO"], $departamentos)
        ),
        array(
...
// Exportar los datos
    ...
    if($tipo_listado == 1){
        ...
        $archivo->textoTitulo    = $textos["REPORTE_SECCIONES"];
        $archivo->textoPiePagina = "";
        $archivo->AddPage();

        $archivo->SetFont('Arial','B',6);
        $tituloColumnas = array($textos["CODIGO"],$textos["NOMBRE"],$textos["DEPARTAMENTO_EMPRESA"]);
        $anchoColumnas  = array(30,50,50);
        $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
        $archivo->Ln(4);
    }else{
        $archivo       = fopen($nombreArchivo,"a+");
        $titulos_plano = "\"".$textos["CODIGO"]."\";\"".$textos["NOMBRE"]."\";\"".$textos["DEPARTAMENTO_EMPRESA"]."\"\n";//."\";\"".
        fwrite($archivo, $titulos_plano);
    }

    $consulta = SQL::seleccionar(array("secciones_departamentos"),array("*"),$condicion);
    ...
    if(SQL::filasDevueltas($consulta)){
        ...
        while($datos = SQL::filaEnObjeto($consulta)){
            ...
            if($tipo_listado == 1){
                ...
                $archivo->Cell(30, 4, $datos->codigo, 0, 0, "L",true,"",true);
                $archivo->Cell(50, 4, $datos->nombre, 0, 0, "L",true,"",true);
                $archivo->Cell(50, 4, $departamento, 0, 0, "L",true,"",true);
                $archivo->Ln(4);
            }else{
                $contenido = "\"".$datos->codigo."\";\"".$datos->nombre."\";\"".$departamento."\"\n";
