    ...
    SELECT  -- llave de control de prestamos --
            ...
            P.codigo_transaccion_contable  AS codigo_transaccion_contable,
            P.codigo_contable              AS codigo_contable,
            P.sentido                      AS sentido,
            P.valor_total                  AS valor_total,
            P.forma_pago                   AS forma_pago,
            P.codigo_planilla              AS codigo_planilla,
 
            IF((SELECT  SUM(MS.valor_descuento)
                FROM job_movimiento_control_prestamos_empleados AS MS,
                     job_fechas_prestamos_empleados             AS F,
                     job_control_prestamos_empleados            AS P
                WHERE
                     MS.documento_identidad_empleado = F.documento_identidad_empleado  AND
                     P.documento_identidad_empleado = MS.documento_identidad_empleado AND
                     MS.consecutivo_fecha_pago       = F.consecutivo                   AND
                     MS.fecha_generacion_control     = F.fecha_generacion              AND
                     MS.concepto_prestamo            = F.concepto_prestamo             AND
                     MS.fecha_pago                   = F.fecha_pago
