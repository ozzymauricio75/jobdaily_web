...
if(!empty($url_generar)){
    ...
    if(!$error_continuar){
        ...
        $formularios["PESTANA_BASICA"] = array(
            ...
            array(
                ...
                HTML::campoOculto("codigo_plan_contable",$codigo_contable).
    ...
    // Determino que sucursales no se les ha generado
    ...
    if($forma_periodo_pago_activo=="0"){
        ...
        foreach($forma_sucursales as $codigo_sucursal){
            ...
            // Obtengo todo los empleados donde esten por sucursales y que esten activos////
            ...
            if(SQL::filasDevueltas($consulta_ingreso_empleados) != 0){
                while($datos_ingreso_empleados = SQL::filaEnObjeto($consulta_ingreso_empleados)){
                    ...
                    if(SQL::filasDevueltas($consulta_sucursal_contrato_empleados)){
                        ...
                        $sucursal_contrato     = SQL::filaEnObjeto($consulta_sucursal_contrato_empleados);
                        ...
                        $condicion  = "codigo_empresa='$datos_ingreso_empleados->codigo_empresa' AND documento_identidad_empleado='$datos_ingreso_empleados->documento_identidad_empleado'";
                        $condicion .= " AND codigo_planilla='$forma_codigo_planilla' AND fecha_ingreso_empresa='$datos_ingreso_empleados->fecha_ingreso'  AND codigo_sucursal= '$codigo_sucursal'";
                        ...
                        $llave_tabla      = $forma_ano_generacion . '|' . $forma_mes_generacion . '|' . $forma_periodo . '|' . $forma_fecha_pago . '|' . $forma_sucursal_genera . '|' . $sucursal_contrato->documento_identidad_empleado;
                        if(SQL::existeItem("menu_movimientos_contables", "id", $llave_tabla)){
                            ...
                            if(!$insertar){
                                ...
                                $llave_tabla       .= " AND documento_identidad_empleado=\'$sucursal_contrato->documento_identidad_empleado\'";
                                $consecutivo_cheque = (int) SQL::obtenerValor("consecutivo_cheques", "MAX(consecutivo)", "codigo_tipo_documento = '$tipo_documento_cuenta_bancaria' AND codigo_banco = '$codigo_banco' AND numero = '$numero'");
                                ...
                                if(!$insertar){
                                    ...
                                    ////Genero el consecutivo de la tabla forma_pago_planilla_empleado///
                                    ...
                                    $valor_movimiento_debitos  = SQL::obtenerValor("consulta_datos_planilla","sum(valor_movimiento)",$condicion." AND codigo_sucursal='$codigo_sucursal' AND sentido='D' AND documento_identidad_empleado='$sucursal_contrato->documento_identidad_empleado'");
                                    $valor_movimiento_creditos = SQL::obtenerValor("consulta_datos_planilla","sum(valor_movimiento)",$condicion." AND codigo_sucursal='$codigo_sucursal' AND sentido='C' AND documento_identidad_empleado='$sucursal_contrato->documento_identidad_empleado'");
                                    ...
                                    if($valor_movimiento != 0){
                                        ...
                                        $datos = array(
                                            ...
                                            "codigo_empresa_sucursal_contrato"                  => $sucursal_contrato->codigo_empresa,
                                            "documento_identidad_empleado"                      => $sucursal_contrato->documento_identidad_empleado,
                                            "fecha_ingreso_empresa_empleado"                    => $sucursal_contrato->fecha_ingreso,
                                            "fecha_ingreso_sucursal_empleado"                   => $sucursal_contrato->fecha_ingreso_sucursal,
                                            "codigo_sucursal_contrato"                          => $sucursal_contrato->codigo_sucursal,
                                        ...
                                        $insertar = SQL::insertar("forma_pago_planillas_empleado", $datos);
                                        if(!$insertar){
                                            $error    = true;
                                            $mensaje  = $textos["ERROR_ADICIONAR_ITEM"];
                                            $eliminar = SQL::eliminar("consecutivo_documentos",$condicion_consecutivo_documentos);
