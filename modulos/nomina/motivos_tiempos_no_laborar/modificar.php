...
if (!empty($url_generar)) {
    ...
    if (empty($url_id)) {
        ...
        $vistaConsulta = "motivos_tiempo_no_laborado";
...
/*** Validar los datos provenientes del formulario ***/
    ...
    if($url_item == "codigo"){
        $existe = SQL::existeItem("motivos_tiempo_no_laborado","codigo",$url_valor,"codigo != 0 AND codigo != '".$url_id."'");
        if($existe){
            HTTP::enviarJSON($textos["ERROR_EXISTE_MOTIVO"]);
        }
    }else if($url_item == "descripcion"){
        $existe = SQL::existeItem("motivos_tiempo_no_laborado","descripcion",$url_valor,"codigo != 0 AND codigo != '".$url_id."'");
        if($existe){
    ...
    if(empty($forma_codigo) || $codigo == 0){
    ...
    } else if(SQL::existeItem("motivos_tiempo_no_laborado","codigo",$forma_codigo,"codigo != 0 AND codigo != '".$forma_id."'")){
        $error = true;
        $mensaje = $textos["ERROR_EXISTE_MOTIVO"];
    } else if(SQL::existeItem("motivos_tiempo_no_laborado","descripcion",$forma_descripcion,"codigo != 0 AND codigo != '".$forma_id."'")){
        $error = true;
        ...
        $modificar = SQL::modificar("motivos_tiempo_no_laborado", $datos, "codigo = '".$forma_id."'");
