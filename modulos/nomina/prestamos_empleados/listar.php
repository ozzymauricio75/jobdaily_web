...
// Exportar los datos
    ...
    if (!isset($forma_sucursales)){
        ...
        if ($forma_tipo_listado=="1"){
            ...
            $archivo->textoTitulo    = $textos["REPORTE_PRESTAMOS"]." ".$textos["FECHAS"]." ".$forma_fechas;
        ...
        foreach($forma_sucursales AS $codigo_sucursal){
            ...
            if ($forma_tipo_listado == 1){
                $archivo                 = new PDF("L","mm","Letter");
                $archivo->textoTitulo    = $textos["REPORTE_PRESTAMOS"];
                $archivo->textoCabecera  = $textos["FECHA"].": ".date("Y-m-d");
                $archivo->textoPiePagina = "";
                $archivo->AddPage();

                $tituloColumnas = array($textos["SUCURSAL_LABORA"],$textos["NUMERO_DOCUMENTO"], $textos["EMPLEADO"], $textos["TRANSACCION_CONTABLE_DESCONTAR"], $textos["TRANSACCION_CONTABLE_COBRAR"], $textos["FECHA_PRESTAMO"], $textos["VALOR_PRESTAMO"]);
                $anchoColumnas  = array(30,30,50,45,45,30,30);

                $archivo->generarCabeceraTabla($tituloColumnas, $anchoColumnas);
                $archivo->Ln(4);
            } else {
                $archivo       = fopen($nombreArchivo,"a+");
                $titulos_plano = "\"".$textos["SUCURSAL_LABORA"]."\";\"".$textos["NUMERO_DOCUMENTO"]."\";\"".$textos["EMPLEADO"]."\";\"".$textos["TRANSACCION_CONTABLE_DESCONTAR"]."\";\"".$textos["TRANSACCION_CONTABLE_COBRAR"]."\";\"".$textos["FECHA_PRESTAMO"]."\";\"".$textos["VALOR_PRESTAMO"]."\"\n";
                fwrite($archivo, $titulos_plano);
            }

            $i=0;

            $condicion = "codigo_sucursal = '$codigo_sucursal' AND fecha_generacion BETWEEN '$fecha1' AND '$fecha2'";
            if (!empty($forma_documento_identidad)){
                $condicion .= " AND documento_identidad_empleado = '$forma_documento_identidad'";
            }
            $consulta = SQL::seleccionar(array("consulta_prestamos_empleados"), array("*"),$condicion,"","codigo_sucursal,$orden_empleado,fecha_generacion");
            ...
            if (SQL::filasDevueltas($consulta)) {
                ...
                while($datos = SQL::filaEnObjeto($consulta)) {
                    ...
                    $transaccion_descontar = SQL::obtenerValor("transacciones_contables_empleado","nombre","codigo='".$datos->codigo_transaccion_contable_descontar."'");
                    $transaccion_cobrar    = SQL::obtenerValor("transacciones_contables_empleado","nombre","codigo='".$datos->codigo_transaccion_contable_cobrar."'");
                    $sucursal              = SQL::obtenerValor("sucursales","nombre","codigo='".$datos->codigo_sucursal."'");
                    $valor_prestamo        = "$ ".number_format($datos->valor_total,0);
                    if ($forma_orden_empleado=="1"){
                    ...
                    if($forma_tipo_listado == 1){
                        ...
                        if ($documento_identidad_anterior != $datos->documento_identidad_empleado && $documento_identidad_anterior!="" && empty($forma_documento_identidad)){
                            ...
                            $archivo->Cell(230, 4, $textos["TOTAL_EMPLEADO"], 0, 0, "R", true,"",true);
                            $archivo->Cell(30, 4, "$ ".number_format($total_empleado,0), 0, 0, "R", true,"",true);
                            $archivo->Ln(4);
                            $total_empleado = 0;
                        }
                        ...
                        $archivo->Cell(45, 4, $transaccion_descontar, 0, 0, "L", true,"",true);
                        $archivo->Cell(45, 4, $transaccion_cobrar, 0, 0, "L", true,"",true);
                        $archivo->Cell(30, 4, $datos->fecha_generacion, 0, 0, "C", true,"",true);
                        $archivo->Cell(30, 4, $valor_prestamo, 0, 0, "R", true,"",true);
                        $archivo->Ln(4);
                    }else{
                        $transaccion_descontar = str_replace(";","",$transaccion_descontar);
                        $transaccion_cobrar    = str_replace(";","",$transaccion_cobrar);
                        $sucursal              = str_replace(";","",$sucursal);
                        $empleado              = str_replace(";","",$empleado);

                        $contenido = $sucursal.";".$datos->documento_identidad_empleado.";".$empleado.";".$transaccion_descontar.";".$transaccion_cobrar.";".$datos->fecha_generacion.";".(int)$datos->valor_total."\n";
                        fwrite($archivo,$contenido);
                    }

                    $total_empleado += $datos->valor_total;
                    $total_sucursal += $datos->valor_total;
                ...
                if ($forma_tipo_listado=="1"){
                    ...
                    $archivo->SetFont('Arial',"B",7);
                    $archivo->Cell(230, 4, $textos["TOTAL_SUCURSAL"], 0, 0, "R", true,"",true);
                    $archivo->Cell(30, 4, "$ ".number_format($total_sucursal,0), 0, 0, "R", true,"",true);
                    $archivo->Ln(4);
                }
            }
        }
