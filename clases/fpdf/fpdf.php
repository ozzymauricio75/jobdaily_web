...
function AddPage($orientation='', $format='', $header=true, $piePagina=true){
    //Start a new page
    if($this->state==0)
        $this->Open();
...
function subText($txt,$w){
    $l=strlen($txt);
    $tx2 = substr($txt,0,$l);
    while(($w-1) < $this->GetStringWidth($tx2)){
        $l = $l-1;
        $tx2 = substr($txt,0,$l);
    }
    return $tx2;
...
function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='',$substring = false){
    //Output a cell
    $k=$this->k;
...
function Output($name='', $dest='', $ponerPie=true)
{
